				...
				buttons: [
					@if(auth()->user()->is_admin)
					{
						...
						action: function( e, dt, node, config ) {
							window.events.$emit('createInventory');
						}
					},
					{
						text: 'Edit',
						action: function( e, dt, node, config ) {
							window.events.$emit('editInventory', table.rows({selected: true}).data().toArray());
						},
						enabled: false
					},					
					//Enable the following if delete is allowed
					{
						text: 'Delete',
						action: function( e, dt, node, config ) {
							window.events.$emit('deleteInventory', table.rows({selected: true}).data().toArray());
						},
						enabled: false
					},
					@endif
